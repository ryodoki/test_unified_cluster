...
# -*- coding: utf-8 -*-
"""
Excelの「質問箇所」「質問内容」列が見つからない問題に強い版。
- ヘッダー行の自動検出（上から5行まで探す）
- 全角/半角スペース、改行を無視した曖昧一致
- 同義っぽい列名（「質疑内容」「設問箇所」など）も拾う
- --sheet でシート名指定可（未指定は先頭シート）

使い方:
  python qa_cluster_ja_v2.py input.xlsx output.xlsx
  # 明示指定したい場合
  python qa_cluster_ja_v2.py input.xlsx output.xlsx --col_place 質問箇所 --col_body 質問内容
"""
import argparse
import re
...
# ---------- ユーティリティ ----------
def norm(s: str) -> str:
    """列名の正規化: 空白全削除, 改行削除, 全角空白も削除, 小文字化"""
    s = str(s)
    s = s.replace("\n", "").replace("\r", "").replace("\u3000", " ")
    s = re.sub(r"\s+", "", s)
    return s.lower()
...
def autodetect_header(path: str, sheet_name=None, max_rows=5) -> int:
    """上からmax_rows行まで見て、想定列名が揃う行をヘッダー行として返す。見つからなければ0"""
    for h in range(0, max_rows):
        df = pd.read_excel(path, sheet_name=sheet_name, header=h, nrows=30)
...
def main():
    ...
    ap.add_argument("--sheet", default=None, help="シート名（未指定は先頭）")
    ...
    header_row = autodetect_header(args.input, sheet_name=args.sheet, max_rows=5)
    df = pd.read_excel(args.input, sheet_name=args.sheet, header=header_row)
